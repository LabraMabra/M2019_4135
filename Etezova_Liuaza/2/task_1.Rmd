...
```{r linearizing}
airq_l <- air_quality
airq_l$NMHC.GT. <- air_quality$NMHC.GT.^(0.7)
airq_l$PT08.S1.CO. <- air_quality$PT08.S1.CO.^2
airq_l$PT08.S2.NMHC. <- air_quality$PT08.S2.NMHC.^2
airq_l$PT08.S3.NOx. <- 1/air_quality$PT08.S3.NOx.^2
airq_l$PT08.S4.NO2. <- air_quality$PT08.S4.NO2.^(2.5)
airq_l$PT08.S5.O3. <- air_quality$PT08.S5.O3.^(1.5)
```
...
lapply(airq_l[, !(names(airq_l) %in% c('Date', 'Time', 'C6H6.GT.'))], benzene_lm_summary)
```

### Selected predictors
PT08.S2(NMHC): PT08.S2(NMHC)^2 after linearizing - titanium dioxide,
CO(GT) - carbon monoxide, 
NMHC(GT): NMHC(GT)^0.7 after linearizing - nonmethane hydrocarbons.
They have Adjusted R-squared > 0.84. R-squared for PT08.S2(NMHC)^2 is 0.9996.
```{r}
set.seed(42)
...
ggplot() +
  ggtitle(paste("R-squared = ", summary_PT08S2$adj.r.squared, '\n', "p-value = ", 
                summary_PT08S2$coefficients[,4][[2]])) +
  xlab('PT08.S2(NMHC)^2') + 
  geom_point(data=train, aes_string(x=predictor, y='C6H6.GT.')) +
  geom_point(data=test, aes_string(x=predictor, y='C6H6.GT.'), color='green') + 
  geom_smooth(method='lm', se=F, data=train, aes_string(x=predictor, y='C6H6.GT.')) + 
  geom_point(data=test, aes_string(x=predictor, y='C6H6byPT08S2'), color='red')

predictor <- 'CO.GT.'
ggplot() +
  ggtitle(paste("R-squared = ", summary_CO$adj.r.squared, '\n', "p-value = ", 
                summary_CO$coefficients[,4][[2]])) +
  xlab('CO(GT)') + 
  geom_point(data=train, aes_string(x=predictor, y='C6H6.GT.')) +
  geom_point(data=test, aes_string(x=predictor, y='C6H6.GT.'), color='green') + 
  geom_smooth(method='lm', se=F, data=train, aes_string(x=predictor, y='C6H6.GT.')) + 
  geom_point(data=test, aes_string(x=predictor, y='C6H6byCO'), color='red')

predictor <- 'NMHC.GT.'
ggplot() +
  ggtitle(paste("R-squared = ", summary_NMHC$adj.r.squared, '\n', "p-value = ", 
                summary_NMHC$coefficients[,4][[2]])) +
  xlab('NMHC(GT)^0.7') + 
  geom_point(data=train, aes_string(x=predictor, y='C6H6.GT.')) +
  geom_point(data=test, aes_string(x=predictor, y='C6H6.GT.'), color='green') + 
  geom_smooth(method='lm', se=F, data=train, aes_string(x=predictor, y='C6H6.GT.')) + 
  geom_point(data=test, aes_string(x=predictor, y='C6H6byNMHC'), color='red')
```
