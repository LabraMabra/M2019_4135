...
dataSubset <- function(data, colNames = colnames(data), colIndexes = c(1:ncol(data)), colLogicVector = NULL, rowNames = rownames(data), rowIndexes = c(1:nrow(data))) {
  ...
  columnSums <- colSums(select_if(data, is.numeric), na.rm = T)
  columnMeans <- colMeans(select_if(data, is.numeric), na.rm = T)
  columnSDs <- apply((select_if(data, is.numeric)), 2, sd, na.rm = T)
  freqTable <- apply(select_if(data, negate(is.numeric)), 2, table)
