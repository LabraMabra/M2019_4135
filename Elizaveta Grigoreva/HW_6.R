...
iris_long <- iris_long_before[names(iris_long_before) != "Measure ID"]
str(iris_long)
#Create_plot
...
ggplot(gapminder_2007,aes(x = gdpPercap,y = lifeExp,size = pop,color = continent)) +
  geom_point() +labs(size = "Population", color = "Continent") 


#Task B2
#Scatter+Line for lifeEXP and combine it
A <- ggplot(gapminder %>% group_by(year, continent) %>% summarise(meanLifeExp = mean(lifeExp)),aes(x = year, y = meanLifeExp,color = continent)) +geom_point() 
B <- ggplot(gapminder %>% group_by(year, continent) %>% summarise(meanLifeExp = mean(lifeExp)),aes(x = year, y = meanLifeExp,color = continent)) +geom_line() 
